...
//		$Day	=	strtotime($Date);
	...
	$pdf->AddFont('angsa','','angsa.php');
	$pdf->SetFont('angsa','',12);
	$pdf->Cell(1,60,iconv( 'UTF-8','TIS-620','วันที่   '.$strDay." ".$strMonthThai." ".$strYearCut."".$strYear),0,0,"L");
	$pdf ->	Ln(2);
	
	$pdf->AddFont('angsa','','angsa.php');
	$pdf->SetFont('angsa','',12);
	$pdf->Cell(0,70,iconv( 'UTF-8','TIS-620','เลขประจำตัวผู้ป่วย   '. $CusNo),0,0,"L");
	$pdf ->	Ln(0);
	
	$pdf->AddFont('angsa','','angsa.php');
	$pdf->SetFont('angsa','',12);
	$pdf->Cell(115,70,iconv( 'UTF-8','TIS-620','ได้รับเงินจาก   '.$Point."  ".$CusName."  ".$CusSurname),0,0,"C");
